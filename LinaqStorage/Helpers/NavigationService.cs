...
namespace LinaqStorage.Helpers
{
    public class ViewNavigationService : INavigationService
    {
        ...
        public async Task InsertNewRootPage(string pageKey, object parameter)
        {
            var page = GetPage(pageKey, parameter);
            CurrentNavigationPage.Navigation.InsertPageBefore(page, CurrentNavigationPage.RootPage);
            await CurrentNavigationPage.Navigation.PopAsync();
        }

        public async Task InsertNewRootPage(string pageKey)
        {
            await InsertNewRootPage(pageKey, null);
        }

        public async Task UpdagePagesLanguage()
        {
            await CurrentNavigationPage.Navigation.PopToRootAsync();
            await InsertNewRootPage("MainPage");
            await CurrentNavigationPage.Navigation.PopAsync();
